...
envsubst < ../ironmaiden/apps/kube-system/cilium/app/values.yaml > cni/values.yaml
kustomize build --enable-helm cni | kubectl apply -f -
rm cni/values.yaml
rm -rf cni/charts

rm -rf kubelet-csr-approver/charts
envsubst < ../ironmaiden/apps/system-controllers/kubelet-csr-approver/app/values.yaml > kubelet-csr-approver/values.yaml
